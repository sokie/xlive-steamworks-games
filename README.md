...
| --- | --- | --- |
| Street Fighter X Tekken | `games/sfxt` | `sfxt-gfwl` (GFWL build: SDK dll + the SDK steam_api.dll), `sfxt-steam` (Steam build: adds the `steam_api.dll` shim that gives the 2012 exe its SDK 1.19 exports over the SDK dll). `kit/` packs an owner test kit |
...
2. `games/<title>/xlive_steamworks.json`: start from `python sdk/tools/spa2steam.py Game.exe out/`,
